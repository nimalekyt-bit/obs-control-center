...
function scenesPage() {
  ...
  <section class="scene-studio"><aside class="scene-browser"><div class="scene-pane-title"><span>Сцены OBS</span><b>${scenes.length}</b></div>${scenes.map(scene => `<button class="scene-select ${scene.name === selected.name ? 'is-active' : ''}" data-scene-select="${esc(scene.name)}"><span class="scene-select-icon">${icon('scenes')}</span><div><b>${esc(scene.name)}</b><span>${scene.items.length} источников · ${scene.items.filter(item => item.managed).length} от OCC</span></div>${scene.name === snapshot.obs.currentProgramSceneName ? '<i>LIVE</i>' : ''}</button>`).join('')}</aside>
    <div class="scene-workbench"><div class="scene-canvas-head"><div><span class="live-dot"></span><b>${esc(selected.name)}</b>${selected.name === snapshot.obs.currentProgramSceneName ? '<small>Сейчас в эфире</small>' : '<small>Предпросмотр сцены</small>'}</div><span>${snapshot.obs.video?.baseWidth || 1920} × ${snapshot.obs.video?.baseHeight || 1080}</span></div>
      ${managedItems.length ? `<div class="scene-bulk-actions"><span>${icon('shield')}Только ${managedItems.length} ${pluralRu(managedItems.length, 'источник OCC', 'источника OCC', 'источников OCC').replace(/^\d+\s/,'')}</span><div><button data-obs-scene-action="undo-scene" data-scene="${esc(selected.name)}" ${management ? '' : 'disabled'}>${icon('refresh')}Отменить групповое</button><button data-obs-scene-action="scene-visibility" data-enabled="false" data-scene="${esc(selected.name)}" ${management ? '' : 'disabled'}>Скрыть OCC</button><button data-obs-scene-action="scene-visibility" data-enabled="true" data-scene="${esc(selected.name)}" ${management ? '' : 'disabled'}>Показать OCC</button></div></div>` : ''}
...
function bindActions() {
  ...
  root.querySelectorAll('[data-obs-scene-action]').forEach(button => button.addEventListener('click', async () => {
    button.disabled = true;
    try { snapshot = await window.controlCenter.obsWidgetAction({ action: button.dataset.obsSceneAction, sceneName: button.dataset.scene, enabled: button.dataset.enabled === 'true' }); render(); loadScenePreview(true); showNotice(button.dataset.obsSceneAction === 'undo-scene' ? 'Групповое действие отменено' : 'Видимость OCC-источников обновлена'); }
